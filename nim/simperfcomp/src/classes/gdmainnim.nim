...
method physics_process(self: MainNim; delta: float64) {.gdsync.} = 
  ...
  var start_time_usec:uint64 = Time.get_ticks_usec()

  # --- Core Calculation Loop (O(N^2)) ---
  for i in 0 ..< self.agents.len:
    var agent_i = self.agents[i]
    ...
    for j in 0 ..< self.agents.len:
      ...
      var vec_diff:Vector2 = agentj.global_position - agenti.global_position
      var dist_sq = vec_diff.lengthSquared()
      ...
      if dist_sq < self.interaction_radius * self.interaction_radius and dist_sq > 0.001 : # Avoid division by zero if agents overlap perfectly
        ...
        var separation_direction:Vector2 = -vec_diff / sqrt(dist_sq) # Points away from j
        ...
        var force_magnitude:float32 = self.separation_strength / (sqrt(dist_sq) + 1.0)
  ...
  var end_time_usec:uint64 = Time.get_ticks_usec()
  var duration_usec:uint64 = end_time_usec - start_time_usec
  ...
  if not self.calculation_times.is_empty():
    var reduced_calc_times: float32 = self.calculation_times.reduce(self.callable("array_sum"), variant 0.0).get(float32)
  ...
  for agent in self.forces_to_apply.keys():
  ...
  # --- Update UI ---
  var fps = Performance.get_monitor(Performance_Monitor.timeFps)
